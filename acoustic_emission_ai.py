...
def generate_client_data(seed, n_samples):
...
def federated_average(client_weights, client_sizes):
    ...
    avg = {
        'coef': sum(w['coef'] * s / total for w, s in zip(client_weights, client_sizes)),
        'intercept': sum(w['intercept'] * s / total for w, s in zip(client_weights, client_sizes))
    }
    return avg
...
def run_federated_learning():
    ...
    clients = [
        {"name": "FleetRobotics Singapore", "samples": 300, "seed": 1},
        {"name": "ABS Singapore",           "samples": 200, "seed": 2},
        {"name": "Offshore Operator A",     "samples": 150, "seed": 3},
        {"name": "Port Authority Client",   "samples": 250, "seed": 4},
    ]
    ...
    X_all, y_all = [], []
    for c in clients:
        X, y = generate_client_data(c["seed"], c["samples"])
        c["X"] = X
        c["y"] = y
        X_all.append(X)
        y_all.append(y)
    ...
    X_test, y_test = generate_client_data(99, 500)
    X_test_scaled = scaler.fit_transform(X_test)
    for c in clients:
        c["X_scaled"] = scaler.transform(c["X"])

    # Baseline: single client
    print("\nBaseline: Training on single client only (FleetRobotics)...")
    baseline = LogisticRegression(max_iter=1000, random_state=42)
    baseline.fit(clients[0]["X_scaled"], clients[0]["y"])
    ...
    global_model.fit(clients[0]["X_scaled"], clients[0]["y"])
    ...
    for round_num in range(1, 11):
        ...
        for c in clients:
            ...
            local.fit(c["X_scaled"], c["y"])
            client_weights.append(get_weights(local))
            client_sizes.append(c["samples"])
            client_accs.append(local.score(c["X_scaled"], c["y"]))

        avg_w = federated_average(client_weights, client_sizes)
        set_weights(global_model, avg_w)
