...
export default function App() {
  ...
  // ── Per-tab independent file / preview / detection state ──────────────
  const [tabFiles,    setTabFiles]    = useState({});
  const [tabPreviews, setTabPreviews] = useState({});
  const [tabDet,      setTabDet]      = useState({});
  const [tabEnh,      setTabEnh]      = useState({});

  // Convenience getters for current tab
  const file      = tabFiles[tab]    ?? null;
  const preview   = tabPreviews[tab] ?? null;
  const detResult = tabDet[tab]      ?? null;
  const enhResult = tabEnh[tab]      ?? null;

  // Latest result from ANY scan tab — used by Dashboard, Compliance, Memory
  const latestDetResult = tabDet["scan"] ?? tabDet["hull"] ?? tabDet["pipeline"] ?? tabDet["cable"] ?? null;

  // Convenience setters for current tab
  const setDetResult = (d) => setTabDet(p => ({...p, [tab]: d}));
  const setEnhResult = (d) => setTabEnh(p => ({...p, [tab]: d}));
  ...
  const [demoUser] = useState(() => {
    try { const u = sessionStorage.getItem("nauticai-demo-user"); return u ? JSON.parse(u) : null; }
    catch { return null; }
  });
  ...
  useEffect(() => {
    ...
    if (p.get("demo") === "1") {
      const u = { name: p.get("name")||"", email: p.get("email")||"", whatsapp: p.get("whatsapp")||"" };
      sessionStorage.setItem("nauticai-demo-access","1");
      sessionStorage.setItem("nauticai-demo-user", JSON.stringify(u));
      window.history.replaceState({}, "", window.location.pathname);
    }
    if (sessionStorage.getItem("nauticai-demo-access") !== "1") sessionStorage.setItem("nauticai-demo-access","1");
  }, []);
  ...
  useEffect(() => {
    ...
    setInsp1Sev(""); setInsp2Sev("");
    saveMission(detResult);
  }, [detResult]);
  ...
  const saveMission = async (det) => {
    ...
    } catch(e) { console.error("Mission save:", e); }
  };
  ...
  const handleFile = useCallback((f) => {
    ...
    setTabFiles(p    => ({...p, [tab]: f}));
    setTabPreviews(p => ({...p, [tab]: url}));
    setTabDet(p      => ({...p, [tab]: null}));
    setTabEnh(p      => ({...p, [tab]: null}));
    setError("");
    setPrivacyCountdown(null);
    setDeletionCert(null);
  }, [tab]);
  ...
  const enhance = async () => {
    ...
    try { setEnhResult(await runEnhance(file, enhParams())); }
    catch(e) { setError(e.response?.data?.detail || e.message); }
    setLoading(false);
  };

  const runBoth = async () => {
    if (!file) return; setLoading(true); setError(""); setCompletionAlertSent(false);
    try {
      const [det, enh] = await Promise.all([runDetection(file, detParams()), runEnhance(file, enhParams())]);
      setDetResult(det); setEnhResult(enh);
  ...
  const genVideoPDF = async () => {
    if (!videoFile) return; setError("");
    try {
      await downloadVideoPDF(videoFile, {
        conf_thr: conf, iou_thr: iou, mode: inspMode, vessel_name: vesselName||"Unknown",
        inspector, sample_n: 10, use_clahe: clahe, clahe_clip: claheClip,
        use_green: green, use_edge: edge, turbidity_in: turbLevel, corr_turb: corrTurb, pdf_password: reportPassword,
      });
      setReportsGenerated(n => n+1);
    } catch(e) { setError(e.response?.data?.detail || e.message); }
  };
  ...
  const sendPDFToWa = async () => {
    if (!file) return;
    if (!waTo()) { setWaMessage("Add your WhatsApp number at signup."); setTimeout(()=>setWaMessage(""),4000); return; }
    setSendingWa(true); setWaMessage(""); setError("");
    try {
      const res = await sendPDFToWhatsApp(file, { ...detParams(), vessel_name: vesselName||"Unknown", inspector, to: waTo(), pdf_password: reportPassword });
      setWaMessage(res.sent ? "Report sent." : (res.message||"Could not send."));
    } catch(e) { setWaMessage(e.response?.data?.message || e.message || "Send failed."); }
    setSendingWa(false);
  };
  ...
  const sendVideoPDFToWa = async () => {
    ...
    try {
      const res = await sendVideoPDFToWhatsApp(videoFile, {
        conf_thr: conf, iou_thr: iou, mode: inspMode, vessel_name: vesselName||"Unknown",
        inspector, sample_n: 10, use_clahe: clahe, clahe_clip: claheClip,
        use_green: green, use_edge: edge, turbidity_in: turbLevel, corr_turb: corrTurb, to: waTo(), pdf_password: reportPassword,
      });
  ...
  const sendCompletionAlert = async (source="image") => {
    ...
    try {
      ...
      if (res.sent) { setWaMessage("Alert sent."); setCompletionAlertSent(true); }
      else setWaMessage(res.message||"Could not send.");
    } catch(e) { setWaMessage(e.response?.data?.message || e.message || "Send failed."); }
    setSendingWa(false);
  };

  const analyzeVideo = async () => {
    if (!videoFile) return; setVideoLoading(true); setError(""); setCompletionAlertSent(false);
    try {
      const data = await runVideoDetection(videoFile, {
        conf_thr: conf, iou_thr: iou, mode: inspMode, sample_n: 10,
        use_clahe: clahe, clahe_clip: claheClip, use_green: green,
        use_edge: edge, turbidity_in: turbLevel, corr_turb: corrTurb,
      });
      setVideoResult(data);
  ...
  const buildTwinData = (det) => {
    ...
    return {
      ...
      defects_3d: dets.map((d,i) => ({
        id: i+1, cls: d.cls??d.class??"Unknown",
        severity: d.severity??"Low", conf: d.conf??0,
        pipeline_pos: total===1 ? 50 : Math.round(5 + (i/(total-1))*90),
        angle: (i/Math.max(total,1)) * 2 * Math.PI,
      })),
    };
  };

  useEffect(() => { if (detResult && tab==="pipeline") setShowTwin(true); }, [detResult]);

  const handleTwinLoad = () => {
    if (!detResult || !iframeRef.current) return;
    iframeRef.current.contentWindow.postMessage({ type:"NAUTICAI_DETECTIONS", data: buildTwinData(detResult) }, "*");
  };

  useEffect(() => {
    const onKey = (e) => {
      if (!e.ctrlKey || e.key!=="Enter") return; e.preventDefault();
      if (tab==="scan" && file) runBoth();
      else if (["hull","pipeline","cable"].includes(tab) && file) detect();
  ...
  const imagePageProps = {
    file, preview, fileRef, handleFile, onDragOver, onDragLeave, onDrop,
    detect, genPDF, sendPDFToWa, sendCompletionAlert,
    completionAlertSent, sendingWa, waMessage,
    exportCSV: exportDetectionsCSV, loading, detResult, error,
    privacyCountdown, deletionCert,
    insp1Sev, setInsp1Sev, insp2Sev, setInsp2Sev,
  ...
  return (
    <div className="app-shell">
      {complianceOpen && (
        <ComplianceModal detResult={latestDetResult} vesselName={vesselName} inspector={inspector} onClose={() => setComplianceOpen(false)} />
      )}
      ...
      <aside className="sidebar">
        ...
        <div className="sb-status">
          <span className="pill"><span className={`dot ${health?.status==="ok"?"dot-ok":"dot-warn"}`} />{health?.status==="ok"?"API Online":"API Offline"}</span>
        ...
        <div className="sb-section">
          ...
          <div className="ctrl">
            ...
            <select className="ctrl-select" value={sevFilter} onChange={e=>setSevFilter(e.target.value)}>
              <option>All Detections</option><option>Critical Only</option><option>High+</option><option>Medium+</option>
            </select>
          </div>
        </div>
        <div className="sb-sep" />
        <div className="sb-section">
          ...
          {clahe && <div className="ctrl"><div className="ctrl-header"><label>CLAHE Clip Limit</label><span>{claheClip.toFixed(2)}</span></div><input type="range" className="ctrl-slider" min={1} max={10} step={0.5} value={claheClip} onChange={e=>setClaheClip(+e.target.value)} /></div>}
          ...
          <div className="ctrl"><div className="ctrl-header"><label>Turbidity Level</label><span>{turbLevel.toFixed(2)}</span></div><input type="range" className="ctrl-slider" min={0} max={1} step={0.05} value={turbLevel} onChange={e=>setTurbLevel(+e.target.value)} /></div>
        ...
        <div className="sb-section">
          ...
          <div className="ctrl"><div className="ctrl-header"><label>Vessel Name</label></div><input type="text" className="ctrl-input" placeholder="e.g. MV Neptune Star" value={vesselName} onChange={e=>setVesselName(e.target.value)} /></div>
          <div className="ctrl"><div className="ctrl-header"><label>Inspector</label></div><input type="text" className="ctrl-input" value={inspector} onChange={e=>setInspector(e.target.value)} /></div>
          <div className="ctrl"><div className="ctrl-header"><label>Inspection Mode</label></div>
      ...
      <div className="main-area">
        <header className="topbar">
          <div className="topbar-left">
            ...
            <div className="topbar-title"><h1>NAUTICAI</h1><p>Deep-sea Anomaly Detection Platform</p></div>
          </div>
          <div className="topbar-right">
            <span className="topbar-chip">BETA · INTERNAL DEMO</span>
            <a href={window.location.port==="3000"?"http://localhost:8080/demo.html":"/demo.html"}
              target="_blank" rel="noopener noreferrer"
              className="btn btn-primary btn-sm" style={{textDecoration:"none",color:"inherit"}}>
              Book a demo
            </a>
          </div>
        </header>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={tab} className="page-content" variants={pgVar} initial="enter" animate="center" exit="exit">
            {tab==="scan"     && <ScanPage {...imagePageProps} enhance={enhance} runBoth={runBoth} enhResult={enhResult} />}
            {tab==="hull"     && <HullPage {...imagePageProps} />}
            {tab==="video"    && <VideoPage videoFile={videoFile} videoRef={videoRef} setVideoFile={setVideoFile}
                                  onDragOver={onDragOver} onDragLeave={onDragLeave}
                                  analyzeVideo={analyzeVideo} genVideoPDF={genVideoPDF}
                                  sendVideoPDFToWa={sendVideoPDFToWa} sendCompletionAlert={sendCompletionAlert}
                                  completionAlertSent={completionAlertSent} videoLoading={videoLoading}
                                  sendingWa={sendingWa} waMessage={waMessage} videoResult={videoResult} error={error} />}
            {tab==="pipeline" && <PipelinePage {...imagePageProps}
                                  showTwin={showTwin} setShowTwin={setShowTwin}
                                  iframeRef={iframeRef} handleTwinLoad={handleTwinLoad} />}
            ...
            {tab==="zero"     && <ZeroShotPage zeroShotClasses={zeroShotClasses} setZeroShotClasses={setZeroShotClasses}
                                  file={file} fileRef={fileRef} handleFile={handleFile}
                                  onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop} preview={preview} />}
            {tab==="road"     && <RoadmapPage />}
          </motion.div>
        </AnimatePresence>

        <footer className="app-footer">
          <img src="/nauticai-logo.png" alt="" /><span className="footer-dot">·</span>
          <a href="https://www.nauticai-ai.com" target="_blank" rel="noreferrer">www.nauticai-ai.com</a>
...
function Toggle({ label, checked, onChange }) {
  return (
    <label className="ctrl-toggle">
      <span className="tgl"><input type="checkbox" checked={checked} onChange={onChange} /><span className="tgl-track" /><span className="tgl-knob" /></span>
      <span>{label}</span>
    </label>
  );
}
...
/* ── DropZone ── */
function DropZone({ fileRef, file, preview, handleFile, onDragOver, onDragLeave, onDrop }) {
  return (
    <div className="card mb-20">
      <div className="card-title">Image Upload</div>
      <div className="dropzone" onClick={()=>fileRef.current?.click()} onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}>
        <div className="dz-icon">📷</div>
        <div className="dz-label">{preview?"Click or drop to replace":"Drop underwater image here, or click to browse"}</div>
    ...
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      {slices.map((s,i)=>(
        <g key={i}>
          ...
          {parseInt(s.pct)>=8 && <text x={s.mx} y={s.my} textAnchor="middle" dominantBaseline="middle" fill="white" fontSize="10" fontWeight="700">{s.pct}%</text>}
        </g>
      ))}
    </svg>
  );
...
function ImgPanel({ label, color, src, placeholder }) {
  return (
    <div style={{position:"relative"}}>
      <span style={{position:"absolute",top:10,left:10,zIndex:2,background:"rgba(0,0,0,0.6)",border:`1px solid ${color}44`,color,fontSize:10,fontWeight:700,letterSpacing:1,padding:"3px 8px",borderRadius:4,textTransform:"uppercase"}}>{label}</span>
      {src
...
function DetectionResultsPanel({ detResult, enhResult, preview, exportCSV }) {
  ...
  const sevCount = dets.reduce((acc,d)=>{ const s=d.severity||"Unknown"; acc[s]=(acc[s]||0)+1; return acc; },{});
  const pieData  = [{label:"Critical",value:sevCount["Critical"]??0,color:"#ef4444"},{label:"High",value:sevCount["High"]??0,color:"#f97316"},{label:"Medium",value:sevCount["Medium"]??0,color:"#f59e0b"},{label:"Low",value:sevCount["Low"]??0,color:"#22c55e"}].filter(d=>d.value>0);
        ...
        <div className="card" style={{overflowX:"auto"}}>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr style={{borderBottom:"1px solid rgba(255,255,255,0.08)"}}>
              {["#","CLASS","CONFIDENCE","SEVERITY","BBOX AREA"].map(h=><th key={h} style={{padding:"6px 12px",textAlign:"left",opacity:0.45,fontSize:11,letterSpacing:0.8,fontWeight:600}}>{h}</th>)}
            </tr></thead>
            <tbody>
              {dets.map((d,i)=>(
                <tr key={i} style={{borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                  ...
                  <td style={{padding:"8px 12px"}}>
                    {d.severity?<span style={{display:"inline-flex",alignItems:"center",gap:5,padding:"3px 10px",borderRadius:4,fontSize:11,fontWeight:700,background:sevColor(d.severity)+"22",color:sevColor(d.severity)}}><span style={{width:7,height:7,borderRadius:"50%",background:sevColor(d.severity),display:"inline-block"}} />{d.severity}</span>:"—"}
                  </td>
...
function ImageActionButtons({ file, loading, detect, genPDF, sendPDFToWa, sendCompletionAlert, completionAlertSent, sendingWa, exportCSV, detResult, onOpenCompliance, scanLabel="Run Scan" }) {
  return (
    <div className="row mb-20" style={{gap:10,flexWrap:"wrap"}}>
      <button className="btn btn-primary" disabled={!file||loading} onClick={detect}>{loading&&<span className="spinner"/>} {scanLabel}</button>
      <button className="btn btn-ghost"   disabled={!file}          onClick={genPDF}>📄 PDF Report</button>
      <button className="btn btn-ghost"   disabled={!file||sendingWa} onClick={sendPDFToWa}>{sendingWa?"…":"📱 WhatsApp"}</button>
      {detResult && <>
        <button className="btn btn-ghost" disabled={sendingWa||completionAlertSent} onClick={()=>sendCompletionAlert("image")}>{completionAlertSent?"✓ Alert sent":"🔔 Alert"}</button>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:"#080f1e",border:"1px solid #0d2a4a",borderRadius:12,width:"100%",maxWidth:800,maxHeight:"90vh",overflow:"auto",padding:32}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
          <div><div style={{fontSize:11,opacity:0.5,letterSpacing:1,textTransform:"uppercase"}}>NautiCAI · Compliance</div><h2 style={{fontSize:22,fontWeight:700}}>Regulatory Auto-Fill Report</h2></div>
...
function ScanPage({ file, preview, fileRef, handleFile, onDragOver, onDragLeave, onDrop,
  detect, enhance, runBoth, genPDF, sendPDFToWa, sendCompletionAlert,
  completionAlertSent, sendingWa, waMessage, exportCSV, loading, detResult, enhResult, error,
  ...
  return (
    <>
      <div className="section-header fade-up">
        ...
        <p className="section-desc">Upload an underwater image to run AI-powered anomaly detection, visibility enhancement, and generate PDF reports.</p>
        <div className="section-rule" />
      </div>
      <DropZone {...{fileRef,file,preview,handleFile,onDragOver,onDragLeave,onDrop}} />
      {preview && !detResult && <div className="card mb-20"><div className="card-title">Original Image</div><img src={preview} alt="Original" style={{width:"100%",borderRadius:6}} /></div>}
      <div className="row mb-20" style={{gap:10,flexWrap:"wrap"}}>
        <button className="btn btn-primary" disabled={!file||loading} onClick={detect}>{loading&&<span className="spinner"/>} Run Detection</button>
        <button className="btn btn-ghost"   disabled={!file||loading} onClick={enhance}>Enhance Visibility</button>
        <button className="btn btn-primary" disabled={!file||loading} onClick={runBoth}>Detect + Enhance</button>
        <button className="btn btn-ghost"   disabled={!file}          onClick={genPDF}>📄 PDF Report</button>
        <button className="btn btn-ghost"   disabled={!file||sendingWa} onClick={sendPDFToWa}>{sendingWa?"…":"📱 WhatsApp"}</button>
        {detResult && <>
          <button className="btn btn-ghost" disabled={sendingWa||completionAlertSent} onClick={()=>sendCompletionAlert("image")}>{completionAlertSent?"✓ Alert sent":"🔔 Alert"}</button>
      ...
      <DetectionResultsPanel detResult={detResult} enhResult={enhResult} preview={preview} exportCSV={exportCSV} />
      {detResult && <DisagreementFlag insp1Sev={insp1Sev} setInsp1Sev={setInsp1Sev} insp2Sev={insp2Sev} setInsp2Sev={setInsp2Sev} />}
    </>
  );
}
...
function HullPage({ file, preview, fileRef, handleFile, onDragOver, onDragLeave, onDrop,
  detect, genPDF, sendPDFToWa, sendCompletionAlert, completionAlertSent, sendingWa, waMessage,
  exportCSV, loading, detResult, error, privacyCountdown, deletionCert,
  ...
  return (
    <>
      ...
      <DropZone {...{fileRef,file,preview,handleFile,onDragOver,onDragLeave,onDrop}} />
      {preview && !detResult && <div className="card mb-20"><div className="card-title">Original Image</div><img src={preview} alt="Original" style={{width:"100%",borderRadius:6}} /></div>}
      <ImageActionButtons {...{file,loading,detect,genPDF,sendPDFToWa,sendCompletionAlert,completionAlertSent,sendingWa,exportCSV,detResult,onOpenCompliance}} scanLabel="Run Hull Scan" />
      <StatusBar error={error} waMessage={waMessage} />
      <PrivacyTimer countdown={privacyCountdown} cert={deletionCert} />
      <DetectionResultsPanel detResult={detResult} preview={preview} exportCSV={exportCSV} />
      {detResult && <DisagreementFlag insp1Sev={insp1Sev} setInsp1Sev={setInsp1Sev} insp2Sev={insp2Sev} setInsp2Sev={setInsp2Sev} />}
    </>
  );
}
  ...
  return (
    <>
      ...
      <div className="card mb-20">
        ...
        <div className="dropzone" onClick={()=>videoRef.current?.click()} onDragOver={onDragOver} onDragLeave={onDragLeave}
          onDrop={e=>{e.preventDefault();e.currentTarget.classList.remove("drag-over");setVideoFile(e.dataTransfer.files[0]);}}>
          <div className="dz-icon">🎬</div>
      ...
      {videoResult && (
        <div style={{marginTop:20}}>
          ...
          {videoResult.frames?.length>0 && (
            <div className="card"><div className="card-title" style={{marginBottom:12}}>Sampled Frames</div>
...
/* ═══ PAGE: Pipeline ═══ */
function PipelinePage({ file, preview, fileRef, handleFile, onDragOver, onDragLeave, onDrop,
  detect, genPDF, sendPDFToWa, sendCompletionAlert, completionAlertSent, sendingWa, waMessage,
  exportCSV, loading, detResult, error, showTwin, setShowTwin, iframeRef, handleTwinLoad,
  privacyCountdown, deletionCert, insp1Sev, setInsp1Sev, insp2Sev, setInsp2Sev, onOpenCompliance }) {
  ...
  return (
    <>
      <div className="section-header fade-up">
        ...
        <p className="section-desc">Detect leaks, corrosion, joint failures and coating defects on subsea pipelines. Defects mapped to real spatial coordinates.</p>
        <div className="section-rule" />
      </div>
      <DropZone {...{fileRef,file,preview,handleFile,onDragOver,onDragLeave,onDrop}} />
      {preview && !detResult && <div className="card mb-20"><div className="card-title">Original Image</div><img src={preview} alt="Original" style={{width:"100%",borderRadius:6}} /></div>}
      <div className="row mb-20" style={{gap:10,flexWrap:"wrap"}}>
        <button className="btn btn-primary" disabled={!file||loading} onClick={detect}>{loading&&<span className="spinner"/>} Run Pipeline Scan</button>
        <button className="btn btn-ghost"   disabled={!file}          onClick={genPDF}>📄 PDF Report</button>
        <button className="btn btn-ghost"   disabled={!file||sendingWa} onClick={sendPDFToWa}>{sendingWa?"…":"📱 WhatsApp"}</button>
        {detResult && <>
          <button className="btn btn-ghost" disabled={sendingWa||completionAlertSent} onClick={()=>sendCompletionAlert("image")}>{completionAlertSent?"✓ Alert sent":"🔔 Alert"}</button>
      ...
      {detResult && (
        <>
          <div style={{display:"flex",alignItems:"center",gap:0,marginBottom:16,borderRadius:8,overflow:"hidden",border:"1px solid rgba(34,211,238,0.2)",width:"fit-content"}}>
            <button onClick={()=>{ setActiveView("2d"); setShowTwin(false); }}
              style={{padding:"8px 20px",background:activeView==="2d"?"rgba(34,211,238,0.15)":"transparent",border:"none",borderRight:"1px solid rgba(34,211,238,0.2)",color:activeView==="2d"?"#22d3ee":"rgba(255,255,255,0.5)",cursor:"pointer",fontSize:12,fontWeight:activeView==="2d"?700:400}}>
              🗺️ 2D Pipeline Map
            </button>
            <button onClick={()=>{ setActiveView("3d"); setShowTwin(true); }}
              style={{padding:"8px 20px",background:activeView==="3d"?"rgba(34,211,238,0.15)":"transparent",border:"none",color:activeView==="3d"?"#22d3ee":"rgba(255,255,255,0.5)",cursor:"pointer",fontSize:12,fontWeight:activeView==="3d"?700:400}}>
              🧊 3D Digital Twin
            </button>
          </div>
          {activeView==="2d" && <PipelineMap2D detResult={detResult} imgW={detResult.img_width??1280} imgH={detResult.img_height??720} />}
          {activeView==="3d" && showTwin && (
            <div style={{width:"100%",height:600,marginBottom:20}}>
              <iframe ref={iframeRef} src="/pipeline-3d-twin.html" title="3D Digital Twin"
                style={{width:"100%",height:"100%",border:"none",borderRadius:8}} onLoad={handleTwinLoad} />
            </div>
          )}
        </>
      )}
...
/* ═══ PAGE: Cable ═══ */
function CablePage({ file, preview, fileRef, handleFile, onDragOver, onDragLeave, onDrop,
  detect, genPDF, sendPDFToWa, sendCompletionAlert, completionAlertSent, sendingWa, waMessage,
  exportCSV, loading, detResult, error, privacyCountdown, deletionCert,
  insp1Sev, setInsp1Sev, insp2Sev, setInsp2Sev, onOpenCompliance }) {
  return (
    <>
      ...
      <DropZone {...{fileRef,file,preview,handleFile,onDragOver,onDragLeave,onDrop}} />
      {preview && !detResult && <div className="card mb-20"><div className="card-title">Original Image</div><img src={preview} alt="Original" style={{width:"100%",borderRadius:6}} /></div>}
      <ImageActionButtons {...{file,loading,detect,genPDF,sendPDFToWa,sendCompletionAlert,completionAlertSent,sendingWa,exportCSV,detResult,onOpenCompliance}} scanLabel="Run Cable Scan" />
      <StatusBar error={error} waMessage={waMessage} />
      <PrivacyTimer countdown={privacyCountdown} cert={deletionCert} />
      <DetectionResultsPanel detResult={detResult} preview={preview} exportCSV={exportCSV} />
      {detResult && <DisagreementFlag insp1Sev={insp1Sev} setInsp1Sev={setInsp1Sev} insp2Sev={insp2Sev} setInsp2Sev={setInsp2Sev} />}
...
function DashPage({ detResult, videoResult }) {
  const data=detResult||videoResult; const dets=data?.detections||[];
  ...
  const sevCount=dets.reduce((acc,d)=>{ const s=d.severity||"Unknown"; acc[s]=(acc[s]||0)+1; return acc; },{});
  const classCounts=dets.reduce((acc,d)=>{ const c=d.cls??d.class??"Unknown"; acc[c]=(acc[c]||0)+1; return acc; },{});
  ...
  const radarDots=dets.slice(0,5).map((_,i)=>({x:50+28*Math.cos((i/5)*2*Math.PI-Math.PI/2),y:50+28*Math.sin((i/5)*2*Math.PI-Math.PI/2),color:sevColor(dets[i]?.severity)}));
  return (
    <>
      <div className="section-header fade-up">
        ...
        <>
          <div className="card" style={{marginBottom:20}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"140px 1fr 1fr 1fr 1fr 140px",gap:12,alignItems:"stretch"}}>
              ...
              {[["Objects Detected",total,"#22d3ee"],["Anomaly Classes",uniqueCls,"#22d3ee"],["Avg Confidence",`${avgConf}%`,"#f59e0b"],["Mission Grade",data.grade??"—",data.grade==="A"?"#22c55e":data.grade==="B"?"#84cc16":data.grade==="C"?"#f59e0b":"#ef4444"]].map(([l,v,c])=>(
          ...
          <div className="card" style={{marginBottom:16}}>
            ...
            {missions.map((m,i)=>(
              <div key={m.id} style={{display:"grid",gridTemplateColumns:"160px 80px 80px 80px 1fr",gap:12,alignItems:"center",padding:"10px 12px",background:i%2===0?"rgba(255,255,255,0.02)":"transparent",borderRadius:6}}>
                <div><div style={{fontSize:12,fontWeight:600}}>{m.vessel}</div><div style={{fontSize:10,opacity:0.4}}>{new Date(m.date).toLocaleDateString()}</div></div>
                <div style={{textAlign:"center"}}><div style={{fontSize:16,fontWeight:700,color:gc(m.grade)}}>{m.grade}</div><div style={{fontSize:9,opacity:0.4}}>GRADE</div></div>
                <div style={{textAlign:"center"}}><div style={{fontSize:16,fontWeight:700,color:parseFloat(m.risk)<30?"#22c55e":parseFloat(m.risk)<60?"#f59e0b":"#ef4444"}}>{m.risk}%</div><div style={{fontSize:9,opacity:0.4}}>RISK</div></div>
                <div style={{textAlign:"center"}}><div style={{fontSize:16,fontWeight:700,color:"#22d3ee"}}>{m.total}</div><div style={{fontSize:9,opacity:0.4}}>DEFECTS</div></div>
          ...
          {missions.length>=2 && (
            <div className="card">
              ...
              <div style={{position:"relative",height:80,display:"flex",alignItems:"flex-end",gap:4}}>
                {[...missions].reverse().slice(0,12).map((m,i)=>{
...
/* ═══ PAGE: Zero-Shot ═══ */
function ZeroShotPage({ zeroShotClasses, setZeroShotClasses, file, fileRef, handleFile, onDragOver, onDragLeave, onDrop, preview }) {
  const [input,   setInput]   = useState("");
  const [running, setRunning] = useState(false);
  const [results, setResults] = useState([]);
  const addClass = () => { const v=input.trim(); if(!v||zeroShotClasses.includes(v))return; setZeroShotClasses(prev=>[...prev,v]); setInput(""); };
  const runZeroShot = async () => {
    if (!file||zeroShotClasses.length===0) return;
    setRunning(true);
    await new Promise(r=>setTimeout(r,1500));
    setResults(zeroShotClasses.map(cls=>({ cls, found:Math.random()>0.3, conf:(0.5+Math.random()*0.45).toFixed(2) })));
    setRunning(false);
  };
  return (
    <>
        ...
        <div>
          <div className="card" style={{marginBottom:16}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:12}}>
              <input className="ctrl-input" style={{flex:1}} placeholder="e.g. marine growth, anode depletion..."
                value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addClass()} />
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:6,minHeight:40}}>
              ...
              {zeroShotClasses.map(cls=>(
                <span key={cls} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"4px 10px",background:"rgba(34,211,238,0.1)",border:"1px solid rgba(34,211,238,0.3)",borderRadius:20,fontSize:12,color:"#22d3ee"}}>
                  {cls}<button onClick={()=>setZeroShotClasses(prev=>prev.filter(c=>c!==cls))} style={{background:"none",border:"none",color:"#22d3ee",cursor:"pointer",fontSize:14,lineHeight:1}}>×</button>
                </span>
              ))}
            </div>
          </div>
          ...
          <button className="btn btn-primary" disabled={!file||zeroShotClasses.length===0||running} onClick={runZeroShot} style={{width:"100%"}}>
...
const TECH_STACK=[{label:"Detection",value:"Vision AI Engine",color:"#22d3ee"},{label:"Backend",value:"FastAPI",color:"#94a3b8"},{label:"Frontend",value:"React",color:"#60a5fa"},{label:"Cloud",value:"GCP",color:"#4ade80"},{label:"Inference",value:"PyTorch",color:"#f97316"},{label:"Reports",value:"FPDF2",color:"#c084fc"}];
