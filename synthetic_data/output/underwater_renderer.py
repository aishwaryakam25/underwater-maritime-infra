...
def render_scene(index):
    ...
    # Randomize water color per image (different Jerlov water types)
    world = bpy.context.scene.world
    bg = world.node_tree.nodes['Background']
    ...
    for obj in scene.objects:
        if obj.type == 'LIGHT':
            obj.data.energy = random.uniform(100, 400)
    ...
    print(f"Rendered {index+1}/1000 → {img_path}")
...
print(f"Done! 1000 images saved to {OUTPUT_DIR}")